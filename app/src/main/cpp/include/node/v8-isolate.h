 ...
 * completely separate states.  Objects from one self must not be used in
 * other isolates.  The embedder can create multiple isolates and use them in
 * parallel in multiple threads.  An self can be entered by at most one
...
class V8_EXPORT Isolate {
 public:
  ...
  struct V8_EXPORT CreateParams {
     ...
     * entire lifetime of the self.
     */
    const intptr_t* external_references = nullptr;

    /**
     * Whether calling Atomics.wait (a function that may block) is allowed in
     * this self. This can also be configured via SetAllowAtomicsWait.
     */
  ...
  /**
   * Stack-allocated class which sets the self for all operations
  ...
  /**
   * Allocates a new self but does not initialize it. Does not change the
   * currently entered self.
   *
   * Only Isolate::GetData() and Isolate::SetData(), which access the
   * embedder-controlled parts of the self, are allowed to be called on the
   * uninitialized self. To initialize the self, call
   * Isolate::Initialize().
   *
   * When an self is no longer used its resources should be freed
  ...
  /**
   * Creates a new self.  Does not change the currently entered
   * self.
   *
   * When an self is no longer used its resources should be freed
   * by calling Dispose().  Using the delete operator is not allowed.
  ...
  /**
   * Returns the entered self for the current thread or NULL in
   * case there is no current self.
   *
  ...
  /**
   * Return true if this self is currently active.
   **/
   ...
   * the self is executing long running JavaScript code.
   */
  ...
  /**
   * Sets this self as the entered one for the current thread.
   * Saves the previously entered one (if any), so that it can be
   * restored when exiting.  Re-entering an self is allowed.
   */
  void Enter();

  /**
   * Exits this self by restoring the previously entered one in the
   * current thread.  The self may still stay the same, if it was
  ...
  /**
   * Disposes the self.  The self must not be entered by any
   ...
   * of performing a full self disposal.
   */
  ...
  /**
   * Associate embedder-specific data with the self. |slot| has to be
   * between 0 and GetNumberOfDataSlots() - 1.
   */
  V8_INLINE void SetData(uint32_t slot, void* data);

  /**
   * Retrieve embedder-specific data from the self.
  ...
  /**
   * Return data that was previously attached to the self snapshot via
  ...
  /**
   * Get a call stack sample from the self.
  ...
  /**
   * Returns heap profiler for this self. Will return NULL until the self
   * is initialized.
   */
  ...
  /** Returns the ArrayBuffer::Allocator used in this self. */
  ArrayBuffer::Allocator* GetArrayBufferAllocator();

  /** Returns true if this self has a current context. */
  bool InContext();
  ...
  /**
   * Sets the embedder heap tracer for the self.
   * SetEmbedderHeapTracer cannot be used simultaneously with AttachCppHeap.
   */
  void SetEmbedderHeapTracer(EmbedderHeapTracer* tracer);

  /*
   * Gets the currently active heap tracer for the self that was set with
  ...
  /**
   * Resume execution capability in the given self, whose execution
   * was previously forcefully terminated using TerminateExecution().
   *
   * When execution is forcefully terminated using TerminateExecution(),
   * the self can not resume execution until all JavaScript frames
   ...
   * This method can only be called once per self and must happen during
   * self initialization before background threads are spawned.
   */
  ...
  /**
   * Optional notification that the self switched to the foreground.
   * V8 uses these notifications to guide heuristics.
   */
  void IsolateInForegroundNotification();

  /**
   * Optional notification that the self switched to the background.
   * V8 uses these notifications to guide heuristics.
   */
  ...
  /**
   * Optional notification to tell V8 the current self is used for debugging
  ...
  /**
   * Iterates through all external resources referenced from current self
  ...
  /**
   * Iterates through all the persistent handles in the current self's heap
   * that have class_ids.
   */
  ...
  /**
   * Check if this self is in use.
   * True if at least one thread Enter'ed this self.
   */
  bool IsInUse();

  /**
   * Set whether calling Atomics.wait (a function that may block) is allowed in
   * this self. This can also be configured via
