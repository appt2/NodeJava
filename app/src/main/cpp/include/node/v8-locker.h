 ...
 * to use any given V8 self, see the comments in the Isolate class. The
 * definition of 'using a V8 self' includes accessing handles or holding onto
 * object pointers obtained from V8 handles while in the particular V8 self.
 ...
 * self. V8 guarantees that an self can be locked by at most one thread at
 ...
 * ...
 * {
 *   v8::Locker locker(self);
 *   v8::Isolate::Scope isolate_scope(self);
 *   ...
 *   // Code using V8 and self goes here.
 *   ...
 ...
 * {
 *   v8::Locker locker(self);
 *   Isolate::Scope isolate_scope(self);
 *   // V8 locked.
 *   {
 *     v8::Locker another_locker(self);
...
class V8_EXPORT Locker {
 public:
  ...
  /**
   * Returns whether or not the locker for a given self, is locked by the
   * current thread.
   */
