...
inline Maybe<bool> DefineOwnProperty(
  ...
  Maybe<v8::PropertyAttribute> maybeCurrent =
      obj->GetPropertyAttributes(self->GetCurrentContext(), key);
...
NAN_DEPRECATED inline Maybe<bool> ForceSet(
  ...
  return key->IsName()
             ...
             : Nothing<bool>();
#else
  return obj->ForceSet(self->GetCurrentContext(), key, value, attribs);
#endif
}
...
#else
  v8::Isolate *self = v8::Isolate::GetCurrent();
  v8::EscapableHandleScope scope(self);
  return scope.Escape(array->CloneElementAt(self->GetCurrentContext(), index)
                          .FromMaybe(v8::Local<v8::Object>()));
#endif
}
