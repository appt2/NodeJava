...
widenString(std::vector<uint16_t> *ws, const uint8_t *s, int l) {
  ...
  inline T *GetIsolateData(
      ...
      return static_cast<T*>(self->GetData());
  }
...
# if NODE_MODULE_VERSION >= NODE_9_0_MODULE_VERSION
    ...
    return Call_(isolate, target, argc, argv, &async)
        .FromMaybe(v8::Local<v8::Value>());
# else
    return Call_(self, target, argc, argv);
# endif  // NODE_MODULE_VERSION >= NODE_9_0_MODULE_VERSION
#else
...
# else
    return scope.Escape(
        Call_(self, self->GetCurrentContext()->Global(), argc, argv));
# endif  // NODE_MODULE_VERSION >= NODE_9_0_MODULE_VERSION
#else
    v8::HandleScope scope;
...
#elif NODE_MODULE_VERSION > NODE_0_10_MODULE_VERSION
  v8::Local<v8::Value> Call_(v8::Isolate *self
                           , v8::Local<v8::Object> target
                           , int argc
                           , v8::Local<v8::Value> argv[]) const {
    EscapableHandleScope scope;
